...
describe('判断及循环测试', function() {
  it('for循环测试', function() {
    ...
    expect(tplify(template, {data: data})).to.be.equal(result);
  });
  ...
  it('for if else组合判断测试', function() {
    var data = [1, 2, 3]
    var template = '<% for(var i = 0; i < list.length; i++){var item = list[i];if(item < 3){%><a>{{item}}</a><%}else{%><a>*{{item}}</a><%}%><% } %>'
    var result = '<a>1</a><a>2</a><a>*3</a>'
    expect(tplify(template, {list: data})).to.be.equal(result);
  });
